...
public class TerrainScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        coll.isTrigger = true;
        coll.size = new Vector3(terrainSettings.MapSize.x, 2048f, terrainSettings.MapSize.z);
        ...
        if (transform.childCount >= 1)
        {
            if (tracker.transform.localPosition.x >= terrainSettings.MapSize.x - terrainSettings.EdgeDistance)
            {
                if (!Neighbors.ContainsKey(TerrainExtensions.Direction.Right))
                {
                    ...
                    {
                        GameObject NewTerrain = TerrainExtensions.CreateTerrain(Pos, terrain, TerrainExtensions.Direction.Right,null,TerrainBiome.Biomes.Planes);
            ...
            if (tracker.transform.localPosition.x <= terrainSettings.EdgeDistance)
            {
                ...
                if (!Neighbors.ContainsKey(TerrainExtensions.Direction.Left))
                {
                    ...
                    if (!GameMaster.gameMaster.terrainManager.TerrainPositions.ContainsKey(Pos))
                    {
                        GameObject NewTerrain = TerrainExtensions.CreateTerrain(Pos, terrain, TerrainExtensions.Direction.Left,null, TerrainBiome.Biomes.Planes);
            ...
            if (tracker.transform.localPosition.z <= terrainSettings.EdgeDistance)
            {
                ...
                if (!Neighbors.ContainsKey(TerrainExtensions.Direction.Down))
                {
                    ...
                    if (!GameMaster.gameMaster.terrainManager.TerrainPositions.ContainsKey(Pos))
                    {
                        GameObject NewTerrain = TerrainExtensions.CreateTerrain(Pos, terrain, TerrainExtensions.Direction.Down, null, TerrainBiome.Biomes.Planes);
